class HScrollbar {
  ...
  HScrollbar (float x, float y, float w, float h) {
    ...
    sliderPosition = barWidth/2 - barHeight/2 + xPosition;
  ...
  void display(PGraphics pg_scrollbar)
  {
    pg_scrollbar.beginDraw();
    pg_scrollbar.noStroke();
    pg_scrollbar.fill(204);
    pg_scrollbar.rect(0, 0, barWidth, barHeight);
    if (mouseOver || locked) {
      pg_scrollbar.fill(0, 0, 0);
    }
    else {
      pg_scrollbar.fill(102, 102, 102);
    }
    pg_scrollbar.rect(sliderPosition-xPosition, 0, barHeight, barHeight);
    pg_scrollbar.endDraw();
    image(pg_scrollbar, xPosition, yPosition);
  }
